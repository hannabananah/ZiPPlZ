...
export default function Policy() {
  ...
  const context = `### 제 1 장 총칙

 제 1 조 (목적)

본 약관은 (주)집플리즈(이하 “회사”라 합니다)이 운영하는 웹사이트 ‘ZIP-PlZ’ (https://i11a602.p.ssafy.io/ ) (이하 “웹사이트”라 합니다)에서 제공하는 온라인 서비스(이하 “서비스”라 한다)를 이용함에 있어 사이버몰과 이용자의 권리, 의무 및 책임사항을 규정함을 목적으로 합니다.
...
2. 회사는 아래 사항에 해당하는 경우에 그 제한사유가 해소될 때까지 승낙을 유보할 수 있다.가. 서비스 관련 설비에 여유가 없는 경우나. 기술상 지장이 있는 경우다. 기타 회사 사정상 필요하다고 인정되는 경우
3. 회사는 아래 사항에 해당하는 경우에 승낙을 하지 않을 수 있다.가. 다른 사람의 명의를 사용하여 신청한 경우나. 이용자 정보를 허위로 기재하여 신청한 경우다. 사회의 안녕질서 또는 미풍양속을 저해할 목적으로 신청한 경우라. 기타 회사가 정한 이용신청 요건이 미비한 경우
...
본 약관은 2024년 08월 02일부터 적용한다.`;
  ...
  return (
    <div className="flex items-start justify-center p-6 mt-8 mb-16">
      <div className="w-full">
        ...
        <div className="w-full px-6 py-4 mt-4  font-bold text-zp-xs bg-zp-sub-color rounded-zp-radius-big drop-shadow-zp-slight">
          <div
            className={`overflow-hidden transition-all duration-500 ${isExpanded ? 'h-auto' : 'h-[calc(28rem)]'} `}
          >
            <ReactMarkdown
              children={context}
              ...
              components={{
                ...
                h3: ({ node, ...props }) => (
                  <h3
                    style={{ fontSize: '1.25em', margin: '0.5em 0' }}
                    {...props}
                  />
                ),
              }}
          ...
          <div className="flex justify-end text-zp-gray">
            <button
              onClick={handleToggleExpand}
              className={`p-2 hover:underline ${isExpanded ? 'bg-gray-700' : ''}`}
            >
              {isExpanded ? '접기' : '더보기'}
            </button>
          </div>
